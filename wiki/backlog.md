...
## Now (in progress)
- *(nothing actively running)* — last completed: the **MiniCPM5-1B think-vs-no-think agentic suite** (2026-06-21) — home-automation v0.4 **Think 0.632 / pass^3 0.210 vs No-Think 0.474 / 0.158** (Think modestly ahead) + email-triage v0.3 Think 0.833. Finding: **mixed / task-dependent, weak either way** (pass^3 0.16–0.21 on HA); the `_no_tool` flail is the 1B's ceiling in both modes, not a Think artifact. Revises the earlier "`--no-think` better" call. Prior same-day batch: the HA v0.4 redesign, the think-axis schema v4, and the MiniCPM5 ET no-think promotion + e5 parity.
