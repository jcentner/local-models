...
## Now (in progress)
- *(nothing actively running)* — last completed: **harness concurrency** — overlap the Copilot-CLI user-sim/judge waits with GPU generation (`--concurrency auto`); **email-triage v0.3 qwen3.5:4b 123.7s → 81.7s (~34%)**, scoring unchanged; phased + gpt-5.5-reviewed, selftest 215 (2026-06-21). Prior: the **MiniCPM5-1B think-vs-no-think agentic suite** — HA v0.4 **Think 0.632 / 0.210 vs No-Think 0.474 / 0.158** + ET v0.3 Think 0.833; **mixed / task-dependent, weak either way**.
...
- **Re-baseline all models on home-automation v0.4** (k=3, **with `--judge-messages`** so the h5/h17/h18 message content is graded) — the v0.4 grounding + double-confirm redesign means prior h5 scores aren't comparable; this **subsumes** re-running any model still on an older HA version. **Only qwen3.5:4b + gemma remain (still on HA v0.3)**; MiniCPM5 HA v0.4 is already done in both think modes **with `--judge-messages`** (results.csv: Think 0.632 / No-Think 0.474).
