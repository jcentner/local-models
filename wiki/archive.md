...
| 3b | Bonsai F16 masters (**deferred** — Jake, 2026-08-14) | `Bonsai-27B-F16.gguf` + `Ternary-Bonsai-27B-F16.gguf` | 2× 50.1 GiB | The QAT-trained weights — unique artifacts nobody can regenerate from Qwen3.6. Deferred with the rest: the point of Bonsai here is the low-bit deployment quants (archived); revisit if extra cheap storage lands |
| 3c | Bonsai 2 27B set (**queued 2026-09-18**; torrent was off) | `prism-ml/Ternary-Bonsai-2-27B-gguf`: `PTQ1_0` (5.54) + `PQ2_0` (6.71) + mmproj-Q8_0 (0.59); fork release `prism-b10685-7dffb15` Linux CUDA 12.8 + Windows CUDA archives; refresh the `Bonsai-demo` clone. F16 master (50.1 GiB) deferred like 3b | ~13 GiB | Same startup risk as item 3; the files are useless without the fork binary, so archive the binary with them. A working copy of `PTQ1_0` is on daedalus (`~/models/prism-ml/`, sha256 verified against HF LFS 2026-09-18) |
