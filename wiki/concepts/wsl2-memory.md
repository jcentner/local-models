...
# WSL2 memory cap (why WSL only sees ~15 GB)

A practical gotcha that directly limits which models run here.

## The finding

This machine has **32 GB** of host RAM, but inside WSL2:
...
WSL2 only exposes **~15 GB**. By default WSL2 caps guest memory at roughly **50%
of host RAM** (or 8 GB, whichever is higher). No `.wslconfig` was present, so the
default applied.
